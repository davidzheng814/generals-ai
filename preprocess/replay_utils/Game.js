...
Game.prototype.printSelf = function() {
  var data = [];
  for (var i = 0; i < 10; ++i) {
    var a = [];
    data.push(a);
    for (var j = 0; j < this.map.height; ++j) {
      var b = [];
      a.push(b);
      for (var k = 0; k < this.map.width; ++k) {
        b.push(0);
      }
    }
  }
  ...
  for (var i = 0; i < this.generals.length; ++i) {
    var ind = this.generals[i];
    var k = ind % this.map.width;
    var j = (ind - (ind % this.map.width)) / this.map.width;
    data[i][j][k] = 1;
  }

  for (var i = 0; i < this.cities.length; ++i) {
    var ind = this.cities[i];
    var k = ind % this.map.width;
    var j = (ind - (ind % this.map.width)) / this.map.width;
    if (this.map._map[ind] < 0) {
      data[7][j][k] = this.map._armies[ind];
    } else if (this.map._map[ind] == 0) {
      data[8][j][k] = this.map._armies[ind];
    } else if (this.map._map[ind] == 1) {
      data[9][j][k] = this.map._armies[ind];
    }
  }

  for (var i = 0; i < this.map.height; ++i) {
    for (var j = 0; j < this.map.width; ++j) {
      var ind = i * this.map.width + j;
      if (this.map._map[ind] == 0 && this.map._armies[ind] == 1) {
        data[2][i][j] = 1;
      } else if (this.map._map[ind] == 1 && this.map._armies[ind] == 1) {
        data[3][i][j] = 1;
      } else if (this.map._map[ind] == 0 && this.map._armies[ind] >= 2) {
        data[4][i][j] = this.map._armies[ind];
      } else if (this.map._map[ind] == 1 && this.map._armies[ind] >= 2) {
        data[5][i][j] = this.map._armies[ind];
      } else if (this.map._map[ind] == -2) {
        data[6][i][j] = 1;
      }
    }
  }

  console.log(this.turn+";"+JSON.stringify(data));
};
